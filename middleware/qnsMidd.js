...
const havePermision = async (ctx, next) => {
    ...
    if (role === 'member') {
        ...
        if (qns != 1) {
            ctx.status = 400;
            ctx.body = { msg: "you can not make change in other person question." };
            return;
        }
    } else {
        ...
        if (qns != 1) {
            ctx.status = 400;
            ctx.body = { msg: "you can not change in other orgenization question." };
            return;
        }
    }
    await next()
...
const canChange = async (ctx, next) => {
    ...
    if (count > 0) {
        ctx.status = 400;
        ctx.body = { msg: 'Now you can not do any operation with this question.' };
        return;
    }
    await next()
}
